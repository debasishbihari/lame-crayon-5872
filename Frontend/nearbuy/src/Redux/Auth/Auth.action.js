  ...
  async (dispatch) => {
    ...
    try {
      let res = await axios.post(
        "https://nearbuy-mock-server.onrender.com/users",
        userData
      );
...
export const Get_USER_item = () => async (dispatch) => {
  ...
  try {
    let respnce = await axios.get(`https://nearbuy-mock-server.onrender.com/users`);
...
export const REMOVE_USER_item = (id) => async (dispatch) => {
  ...
  try {
    let res = await axios.delete(
      `https://nearbuy-mock-server.onrender.com/users/${id}`
    );
